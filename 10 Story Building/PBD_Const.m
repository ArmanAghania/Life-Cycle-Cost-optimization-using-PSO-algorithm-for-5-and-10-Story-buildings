...
load  NonlinearResults/disp1040.out;
...
drift = [drift1 , drift2, drift3, drift4, drift5, drift6, drift7, drift8, drift9, drift10];

indIO = ceil(dt5050/dt_incr);
indLS = ceil(dt1050/dt_incr);

if ceil(dt250/dt_incr) > size(drift,1)
    indCP = size(drift,1);
else
...
for iii = 1:NS/2
    for jjj = 1:2
        cid(1,Cntr)     = Cntri;
        cid(1,Cntr + 1) = Cntri + 5;
        cid(1,Cntr + 2) = Cntri + 5;
        cid(1,Cntr + 3) = Cntri;
        Cntr  = Cntr + 4;       
    end
    Cntri = Cntri + 1;
